...
Q2a <- ggplot(ipip,aes(x=logMedInc,y=BMI, color=gender))+
    ...
    theme_minimal()+
    labs(title="BMI x Income by Gender",x="Log Median Income", y = "BMI", colour="Gender")
Q2a
ggsave('figures/Q2a.pdf',units='in',width=4,height=5)
#customizations: bigger geom point size, plot color theme, point transparency (alpha), labels
...
Q3 <- ggplot(ipip)+
    aes(x=gender, y= logMedInc, fill= exer, weight =logMedInc) + 
    stat_summary(fun.y=mean,  geom="bar",alpha = .7, position=position_dodge(width=.9)) + 
    stat_summary(fun.data=mean_cl_boot, geom="errorbar",width =.3, position=position_dodge(width=.9))+
    coord_cartesian(ylim = c(10, 12))+
    labs(title="Gender Relationships Income & Excercise Frequency",x="Gender", y = "Median Income", colour="Gender")+
...
Q4 <- ggplot(ipip.l, aes(x=BMI_cat, y= value, group=gender, colour= gender))+
    ...
    labs(title="Mean Trait Score by BMI Category",x="BMI Category", y = "Mean Trait Score", colour="Gender")+
    scale_color_viridis(discrete = TRUE, option = "P")+
    theme_dark()
Q4
...
Q5 <- ggplot(ipip.g, aes(x=BMI_cat, y=traitm, group=trait, colour=trait)) +
    ...
    theme_light()+
    labs(title="Mean Trait Score by BMI Category",x="BMI Category", y = "Mean Trait Score")
Q5
